...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    console.log("🔍 Debugging email scheduling system...");
    ...
    console.log("🌍 Kurdistan time:", now.toLocaleString('en-US', { timeZone: 'Asia/Baghdad' }));
    ...
    const analysis = {
      currentTime: {
        utc: now.toISOString(),
        kurdistan: now.toLocaleString('en-US', { timeZone: 'Asia/Baghdad' })
      },
      functionCheck,
      scheduledEmails: {
    ...
    console.log("📊 Scheduling debug results:", JSON.stringify(analysis, null, 2));
    ...
    console.error("❌ Error in debug-scheduling function:", error);
    return new Response(
